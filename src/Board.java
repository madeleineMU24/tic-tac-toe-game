    ...
    public Board() {
        ...
        for (int i = 0; i < 3; i++) { //raderna
            for (int j = 0; j < 3; j++) { //kolumnerna, utgår från i och ger varje i 3 stycken j
                ...
                board[i][j] = '-'; //markerar att platesen är "ledig", en tom plats i arrayen
            }
        }
    }
    ...
    public void printBoard() { //skriver ut spelbrädan
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if (j < 2) { //när det är mindre än 2 skriver man ut | i kolumn, däför blir det inte på den tredje
                    System.out.print(" | "); //mycket av de andra är mer design, tex | och ---+--
                }
            }
            System.out.println();
            if (i < 2) { //om det är mindre än 2 skriver man ut ----+ på raden, därför blir det inte på den treje
                System.out.println("----+-----+-----");
            }
